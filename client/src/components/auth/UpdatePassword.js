...
const UpdatePassword = () => {
    ...
    useEffect(() => {
        if (store.auth.updatePasswordErrors) {
            setError(store.auth.updatePasswordErrors)
        }
    }, [store.auth.updatePasswordErrors])
    const formHandler = (e) => {
        e.preventDefault()
        dispatch(updatePassword({ email: store.auth.user && store.auth.user.email, oldPassword, newPassword, confirmNewPassword },history))
    }
    return (
        <>
